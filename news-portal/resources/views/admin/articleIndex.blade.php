...
@section('content')
    <div class="container mx-auto p-6 max-w-6xl">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('admin.articles.create') }}" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 text-base">Create New Article</a>
        </div>
        @if(session('success'))
    ...
    <tr class="border-b hover:bg-gray-50">
        <td class="p-3">
            ...
            @endif
                                ...
                                <td class="p-3">
                                    <span class="inline-block px-3 py-1 rounded text-sm font-semibold {{ $article->status === 'published' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                ...
                                <td class="p-3 flex space-x-2">
                                    <a href="{{ route('articles.show', $article->slug) }}" class="bg-gray-200 text-gray-700 px-3 py-1 rounded text-sm hover:bg-gray-300">View</a>
                                    <a href="{{ route('admin.articles.edit', $article) }}" class="bg-blue-200 text-blue-700 px-3 py-1 rounded text-sm hover:bg-blue-300">Edit</a>
                                    <form action="{{ route('admin.articles.destroy', $article) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this article?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="bg-red-200 text-red-700 px-3 py-1 rounded text-sm hover:bg-red-300">Delete</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
