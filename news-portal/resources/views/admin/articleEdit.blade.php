    ...
    <div class="container mx-auto p-4 max-w-lg">
        ...
        <form action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="title" id="title" class="w-full border rounded p-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 @error('title') border-red-500 @enderror" value="{{ old('title', $article->title) }}" required>
                @error('title')
            ...
            <div class="mb-3">
                ...
                <textarea name="content" id="content" class="w-full border rounded p-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 @error('content') border-red-500 @enderror" rows="4" required>{{ old('content', $article->content) }}</textarea>
                @error('content')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="datetime-local" name="published_at" id="published_at" class="w-full border rounded p-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 @error('published_at') border-red-500 @enderror" value="{{ old('published_at', $article->published_at ? $article->published_at->format('Y-m-d\TH:i') : '') }}">
                @error('published_at')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
                    ...
                    <div class="mt-2">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach($article->image as $image)
                                <div class="relative">
                                    <img src="{{ asset('storage/' . $image) }}" alt="Article Image" class="w-16 h-16 object-cover rounded border">
                                    <button type="button" onclick="removeImage('{{ $image }}', this)" class="absolute top-0 right-0 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs">×</button>
                                </div>
                            @endforeach
                        </div>
            ...
            <div class="flex justify-end space-x-2">
                <a href="{{ route('admin.articles.index') }}" class="bg-gray-200 text-gray-700 px-3 py-1.5 rounded hover:bg-gray-300 text-sm">Cancel</a>
                <button type="submit" class="bg-blue-500 text-white px-3 py-1.5 rounded hover:bg-blue-600 focus:outline-none focus:ring-1 focus:ring-blue-500 text-sm">Update Article</button>
            </div>
        </form>
