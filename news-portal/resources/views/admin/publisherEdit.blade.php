...
@section('content')
    <div class="container mx-auto p-4 max-w-lg">
        ...
        @endif
        <form action="{{ route('admin.publishers.update', $user) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <select name="role" id="role" class="w-full border rounded p-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 @error('role') border-red-500 @enderror" required>
                    <option value="publisher" {{ old('role', $user->hasRole('publisher') ? 'publisher' : '') === 'publisher' ? 'selected' : '' }}>Publisher</option>
                </select>
                @error('role')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="flex justify-end space-x-2">
                <a href="{{ route('admin.publishers.index') }}" class="bg-gray-200 text-gray-700 px-3 py-1.5 rounded hover:bg-gray-300 text-sm">Cancel</a>
