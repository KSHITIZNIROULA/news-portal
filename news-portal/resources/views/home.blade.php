...
@section('content')
    <div class="container mx-auto pt-2 px-2">
        <h1 class="text-2xl font-bold mb-4 text-gray-900 border-b border-gray-200 pb-2">Latest Articles</h1>
        @if($articles->isEmpty())
                        ...
                        <a href="{{ route('articles.show', $article->slug) }}" class="block">
                            @if($article->images->isNotEmpty())
                                <div class="w-full h-40">
                                    <img src="{{ asset('storage/' . $article->images->first()->path) }}" alt="{{ $article->title }}" class="w-full h-full object-cover">
                                </div>
                            @else
